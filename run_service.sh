...
model_path=experiments/llama2.7b.chat.logiqav2.70b-distil.step.dpo.H100.w2.v1.7
step=2000

python -m vllm.entrypoints.openai.api_server --model $model_path/checkpoint-$step \
  --tokenizer $model_path/checkpoint-$step \
  --dtype bfloat16 --served-model-name llama-2-7b-70bdistil-step-dpo-v1.7-cp$step --disable-log-requests
