...
1. We change the direction of decay, where the value of longer response will be decayed considering its smaller space for exploration.
"""
        ...
        for x_i, x in enumerate(item_states):
            ...
            for y_i, y in enumerate(item_states):
                ...
                if step_id_x < step_id_y:
                    val_y = val_y * args.decay ** (step_id_y - step_id_x)
                elif step_id_x > step_id_y:
                    val_x = val_x * args.decay ** (step_id_x - step_id_y)
