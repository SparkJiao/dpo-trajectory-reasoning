...
You can specify different `pos_margin` for data augmented from GSM8K and MATH for processing.
...
def main():
    ...
    parser.add_argument("--reward_file_2", type=str, default=None)
    parser.add_argument("--output_file", type=str)
    parser.add_argument("--best_of", type=int, default=1)
    parser.add_argument("--max_neg_num", type=int, default=100)
    parser.add_argument("--gsm_pos_margin", type=float, default=2.0)
    parser.add_argument("--math_pos_margin", type=float, default=0.5)
    ...
    if args.reward_file_2 is not None:
        rewards += json.load(open(args.reward_file_2, "r"))
    id2reward = {}
    id2full_reward = {}
    cnt = 0
    for item in rewards:
    ...
    for item in data:
        if "MATH" in item["type"]:
            pos_margin = args.math_pos_margin
            math_cnt += 1
        elif "GSM" in item["type"]:
            pos_margin = args.gsm_pos_margin
            gsm_cnt += 1
        else:
        ...
        for chosen in chosen_responses:
            ...
            for pos_id, pos_reward, pos_resp in pos_response_rewards:
                if args.min_neg_step > 0 and len(item["response"][pos_id].split("\n")) < args.min_neg_step:
                    continue
                if chosen[1] - pos_reward > pos_margin:
                    pos2pos.append({
                        "chosen": chosen[2],
    ...
    print("Math pairs", math_pairs)
    print("GSM pairs", gsm_pairs)
    print("Math cnt", math_cnt)
    print("GSM cnt", gsm_cnt)
