...
def best_of_n_filter(item, best_of: int, id2reward: Dict[str, float], eval_fn=eval_last_single_answer):
    incorrect_responses = []
    responses = []
    ...
    for resp_id, (resp, pred) in enumerate(zip(item["response"], item["pred"])):
        ...
        v = eval_fn({"prediction": pred, "answer": item["label"]})
...
def main():
    ...
    parser.add_argument("--eval_fn", type=str, default="gsm8k")
    ...
    parser.add_argument("--step_offset", type=int, default=1)
    ...
    for item in rewards:
        ...
        if args.step_offset > 0:
            probs = probs[:-args.step_offset]
            logits = logits[:-args.step_offset]
    ...
    pos_pair = 0
    for item in data:
        chosen_responses, pos_response_rewards, reject_responses, correct_num = best_of_n_filter(item, args.best_of, id2reward, eval_fn=fns[args.eval_fn])
        for chosen in chosen_responses:
            ...
            if item["id"] in exclude_ids:
                continue
            if args.hard and len(reject_responses) == 0:
                continue
