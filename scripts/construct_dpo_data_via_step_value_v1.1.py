    ...
    for step_id, step in enumerate(raw_steps):
        if nodes[node_pointer]["content"] in step:
            if nodes[node_pointer]["type"] in included_types:
                # sub_sample_content = "\n".join(raw_steps[:step_id])  # FIXME: `:step_id` here is wrong!!!
                sub_sample_content = "\n".join(raw_steps[:(step_id + 1)])
...
def main():
    ...
    for item in data:
        ...
        step_samples = []
        for resp_i in range(len(item["response"])):
            for resp_j in range(len(item["response"])):
        ...
        avg_step_sample_num += len(step_samples) / len(item["response"]) ** 2
