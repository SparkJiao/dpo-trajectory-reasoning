...
"""
This script takes the output file from ```scripts/react_step_union_find_v2.py``` as input, and construct the pairwise samples.

Updates from version 1.0:
    1.  According to the out-degree of each node, consider if we will treat the current node as the end node for construction.
        Current idea is that given a sequence <s_1, a_1, s_2, a_2, ..., s_i, a_i>, if node s_{i-1} holds the same value with a_{i-1} because
        s_{i-1} has only one son-node, i.e., a_{i-1}, then we will treat a_{i-1} as the end node.
        Similar operations can be tracked to the first node with out-degree larger than 1.
        This is to ensure large enough variance to avoid collapse into imitation learning.
"""
...
def parse_step_samples_w_value(response: str, nodes: List[Dict[str, Union[str, Union[float, Dict]]]], included_types: Tuple[str] = ("Thought", "Observation")):
    ...
    for step_id, step in enumerate(raw_steps):
        if nodes[node_pointer]["content"] in step:
            if nodes[node_pointer]["type"] in included_types:
                sub_sample_content = "\n".join(raw_steps[:(step_id + 1)])  # Seems that this is a bug in previous version !!!!!
                if nodes[node_pointer]["value"]["degree"] > 1:
                    accumulated_step_samples.append({
                        "content": sub_sample_content,
                        "value": nodes[node_pointer]["value"]["value"],
                        "length": step_id + 1,
                    })
            node_pointer += 1
...
def main():
    ...
    Remember that first clean the data (especially those generated by llama-2-chat by removing the redundant examples)
    """
    ...
    parser.add_argument("--value_diff", type=float, default=0.1)
